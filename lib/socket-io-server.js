module.exports = function Server(expressInstance, sessionStore) {
  ...
  var bindings = {};
    ...
    client.on('phone', function(phone){
      sendMessage(phone, 'Remote is at http://comorichweb.no.de/demo/remote/'+id);
    });

		client.on('deck-identify', function(msg) {
      bindings[id] = {slides:client}
      client.emit('remote-location', id);
		});
